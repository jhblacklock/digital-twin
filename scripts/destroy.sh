...
if aws s3 ls "s3://$FRONTEND_BUCKET" --profile "$AWS_PROFILE" 2>/dev/null; then
    echo "  Emptying $FRONTEND_BUCKET..."
    aws s3 rm "s3://$FRONTEND_BUCKET" --recursive --profile "$AWS_PROFILE"
else
    echo "  Frontend bucket not found or already empty"
fi

# Empty memory bucket if it exists
if aws s3 ls "s3://$MEMORY_BUCKET" --profile "$AWS_PROFILE" 2>/dev/null; then
    echo "  Emptying $MEMORY_BUCKET..."
    aws s3 rm "s3://$MEMORY_BUCKET" --recursive --profile "$AWS_PROFILE"
else
